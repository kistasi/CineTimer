...
- **`FilmTimerView`** — the main timer screen. Updates every second via `TimelineView`. Disables the idle timer (`UIApplication.shared.isIdleTimerDisabled`) while visible. Shows contextual content per state: countdown cards when upcoming/in-trailers, a large `%` seen + progress bar + remaining time + end time when playing. Pencil toolbar button opens the edit sheet; a Trash toolbar button deletes the film after a `confirmationDialog`, then `dismiss()`es back to the list (same `deleteFilm()` logic as the list: remove Live Activity, delete row, save, reload widgets). `onAppear` also kicks off the Live Activity via `activities.start(for:)` (there is no manual toggle — Live Activities start automatically).
...
- **`CineTimer/FilmActivityManager.swift`** — a `@MainActor` singleton (`ObservableObject`) that requests / updates / ends the activity through ActivityKit. Activities are keyed to a film by `String(describing: film.persistentModelID)` stored in the attributes. It exposes `activitiesEnabled` (`ActivityAuthorizationInfo().areActivitiesEnabled`). Live Activities start **automatically** — there is no manual toggle. Two start paths: `start(for:)` (timer `onAppear`, edit-restart) requests immediately; `ensureActivity(for:at:)` is the **auto** path called from `ContentView` while the list is visible — it starts an activity for any film between its `startTime` and `filmEnd` that isn't already live, and cleans up (`finish`) once a film ends. `finish(for:)` ends an activity at the film's natural end so a re-scheduled film can start a fresh one later. Both start paths silently no-op when the user has turned Live Activities off in Settings (`activitiesEnabled == false`).
  - **Gotcha:** `Activity.activities` does not reflect a just-`request`ed activity synchronously, so `requestActivity` inserts the new ID into `activeFilmIDs` directly rather than relying on `refresh()`.
  - **Gotcha:** `Activity.request` throws `ActivityAuthorizationError.visibility` if the app isn't in the foreground. Because the auto-start timer / `onAppear` can fire mid-transition or while backgrounded, `requestActivity` skips a *new* start when `UIApplication.shared.applicationState == .background` (existing activities can still `update` in the background), and `ContentView` re-runs `syncActivities()` on `scenePhase == .active` so a skipped start is retried as soon as the app is up front. A Live Activity can therefore only be *started* while the app is foregrounded — there is no background/push start path — but it persists on the Lock Screen once running.
...
- **UI tests** (`CineTimerUITests/`) use XCTest / XCUIAutomation and drive real flows (empty state, add, status badge, open timer, delete with confirmation, cancel delete, delete from the timer screen).
